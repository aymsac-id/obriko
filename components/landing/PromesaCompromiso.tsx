...
// KIT DE LANDING — §7 PROMESA DE CIERRE (blueprint: 55 §7 — sección de riesgo cero para el cliente)
// Franja propia compacta (48-64px) bajo la oferta — NO una línea perdida.
// Escudo SVG en chip 60px (jamás emoji), nombre PROPIO de la promesa en acento,
// condición máx 3 líneas (warn a las 30 palabras), piso Hotmart con candado SVG.
// Nunca placeholders ("promesa visible"): se nombra la política concreta o la
...
export interface PromesaCompromisoProps {
  /** Nombre PROPIO ("la Promesa de Cero Compromiso") — se pinta en acento. */
  nombre: string;
  /** Copy MARCADO de la condición — máx 3 líneas (~30 palabras, warn). */
  condicionMarked: string;
  /** Piso legal verificable cuando exista (ej. plazo real configurado en la pasarela). */
  pisoLegal?: string;
...
export function PromesaCompromiso({ nombre, condicionMarked, pisoLegal, icon: Icono = ShieldCheck, id }: PromesaCompromisoProps) {
  warnCopy('Promesa de cierre → condición', condicionMarked, 30);
  const { contenedor, item } = useReveal();

  return (
    <SectionShell id={id} elevacion="elevada" compacta ariaLabel="Promesa de cierre">
      <motion.div
        variants={contenedor}
        initial="hidden"
        whileInView="visible"
      ...
      >
        <motion.div variants={item}>
          {/* La card de la promesa: uno de los 1-3 usos de hairline permitidos por vista */}
