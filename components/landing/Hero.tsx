...
export interface HeroProps {
  ...
  ctaHref: string;
  /** Franja bajo el CTA (posición FIJA de 19 §1). Día 1 sin números: la promesa de cierre. */
      ...
      <div className="mx-auto w-full max-w-[1140px] px-5">
        ...
        <header className="flex h-16 items-center justify-between">
          ...
          {loginHref && (
            <a
              href={loginHref}
              className="rounded-[var(--radius-button)] border border-[color-mix(in_oklab,var(--text-tertiary)_35%,transparent)] px-4 py-2 text-[14px] font-semibold text-[var(--text-primary)] transition-colors hover:bg-[var(--surface)]"
            >
              {loginLabel}
            </a>
          )}
